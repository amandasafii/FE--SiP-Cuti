...
<body class="bg-[#FFFDEB] flex">
    ...
    <div class="flex-1 p-6">
        <div class="max-w-3xl mx-auto bg-white shadow-lg rounded-lg p-6">
            <h2 class="text-2xl font-semibold text-gray-700 mb-6">Edit Data Kajur</h2>

            <form action="{{ url('/kajur/' . $kajur->id_kajur) }}" method="POST" class="space-y-4">
                @csrf
                @method('PUT')

                <div>
                    <label for="id_kajur" class="block font-semibold mb-1">ID Kajur</label>
                    <input type="text" name="id_kajur" id="id_kajur" value="{{ old('id_kajur', $kajur->id_kajur) }}" 
                        class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-red-500" required readonly>
                    @error('id_kajur')
                ...
                <div>
                    ...
                    <input type="text" name="nama_kajur" id="nama_kajur" value="{{ old('nama_kajur', $kajur->nama_kajur) }}"
                ...
                <div>
                    ...
                    <input type="text" name="nidn" id="nidn" value="{{ old('nidn', $kajur->nidn) }}"
                        class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-red-500" required>
                    @error('nidn')
                        <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="text" name="nama_jurusan" id="nama_jurusan" value="{{ old('nama_jurusan', $kajur->nama_jurusan) }}"
                        class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-red-500" required>
                    @error('nama_jurusan')
                        <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="text" name="id_user" id="id_user" value="{{ old('id_user', $kajur->id_user) }}"
                        class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-red-500" required>
                    @error('id_user')
                        <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                    @enderror
